...
function TimeGiver({ //TimeGiver is the main function of the script that takes arguments about rhythm parameters using key-value syntax with defaults as shown below and returns a list of 8 items describing how the lights should look at that moment according to that schedule.
    ...
    if (rise_length_CCT < 2) {var rise_length_CCT_fin = half_day_length} else {var rise_length_CCT_fin = rise_length_CCT};
    if (set_length_CCT < 2) {var set_length_CCT_fin = half_day_length} else {var set_length_CCT_fin = set_length_CCT};
    if (rise_length_scatdist < 2) {var rise_length_scatdist_fin = half_day_length} else {var rise_length_scatdist_fin = rise_length_scatdist};
    if (set_length_scatdist < 2) {var set_length_scatdist_fin = half_day_length} else {var set_length_scatdist_fin = set_length_scatdist};
    ...
    var rise_CCT = ((((hm - wake_time + wake_offset)/rise_length_CCT_fin) ** (rise_slope_CCT_exp2)) * (max_CCT - min_CCT) + min_CCT);
    var set_bright = ((((bed_time - bed_offset - hm)/set_length_bright) ** (set_slope_bright_exp2)) * (max_bright - min_bright) + min_bright);
    var set_CCT = ((((bed_time - bed_offset - hm)/set_length_CCT_fin) ** (set_slope_CCT_exp2)) * (max_CCT - min_CCT) + min_CCT);
    var rise_scatdist = ((((hm - wake_time + wake_offset)/rise_length_scatdist_fin) ** (rise_slope_scatdist_exp2)) * (max_scatdist - min_scatdist) + min_scatdist);
    var set_scatdist = ((((bed_time - bed_offset - hm)/set_length_scatdist_fin) ** (set_slope_scatdist_exp2)) * (max_scatdist - min_scatdist) + min_scatdist);
    ...
    } else if (hm - wake_time + wake_offset > 0 && hm - wake_time + wake_offset - rise_length_CCT_fin <= 0) {CCT_int = rise_CCT_int
    } else if (hm - wake_time + wake_offset > 0 && hm - wake_time + wake_offset - rise_length_CCT_fin > 0 && hm - bed_time + bed_offset + set_length_CCT_fin <= 0) {CCT_int = max_CCT_int
    } else if (hm - wake_time + wake_offset > 0 && hm - wake_time + wake_offset - rise_length_CCT_fin > 0 && hm - bed_time + bed_offset + set_length_CCT_fin > 0 && hm - bed_time + bed_offset <= 0) {CCT_int = set_CCT_int
    } else if (hm - wake_time + wake_offset > 0 && hm - wake_time + wake_offset - rise_length_CCT_fin > 0 && hm - bed_time + bed_offset + set_length_CCT_fin > 0 && hm - bed_time + bed_offset > 0) {CCT_int = min_CCT_int}
    ...
    } else if (hm - wake_time + wake_offset > 0 && hm - wake_time + wake_offset - rise_length_scatdist_fin <= 0) {scatdist_int = rise_scatdist_int
    } else if (hm - wake_time + wake_offset > 0 && hm - wake_time + wake_offset - rise_length_scatdist_fin > 0 && hm - bed_time + bed_offset + set_length_scatdist_fin <= 0) {scatdist_int = max_scatdist_int
    } else if (hm - wake_time + wake_offset > 0 && hm - wake_time + wake_offset - rise_length_scatdist_fin > 0 && hm - bed_time + bed_offset + set_length_scatdist_fin > 0 && hm - bed_time + bed_offset <= 0) {scatdist_int = set_scatdist_int
    } else if (hm - wake_time + wake_offset > 0 && hm - wake_time + wake_offset - rise_length_scatdist_fin > 0 && hm - bed_time + bed_offset + set_length_scatdist_fin > 0 && hm - bed_time + bed_offset > 0) {scatdist_int = min_scatdist_int}
